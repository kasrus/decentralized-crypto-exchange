...
export const subscribeToEvents = (exchange, dispatch) => {
    exchange.on('Cancel', (id, user, tokenGet , amountGet, 
        tokenGive, amountGive, timestamp, event) => {
            const order = event.args
            dispatch({ type: 'ORDER_CANCEL_SUCCESS', order, event })
    })
    exchange.on('Trade', (id, user, tokenGet, amountGet, tokenGive, 
        amountGive, creator, timestamp, event) => {
        const order = event.args
...
// -------------------------------------------
// LOAD USER BALANCE (WALLET & EXCHANGE BALANCES)
//
export const loadBalances = async(exchange, tokens, account, dispatch) => {
    // 18 comes from converting to wei value
    let balance = await ethers.utils.formatUnits(await tokens[0].balanceOf(account), 18);
    dispatch({ type: 'TOKEN_1_BALANCE_LOADED', balance });

    balance = ethers.utils.formatUnits(await exchange.balanceOf(tokens[0].address, account), 18);
    dispatch({ type: 'EXCHANGE_TOKEN_1_BALANCE_LOADED', balance });

    balance = await ethers.utils.formatUnits(await tokens[1].balanceOf(account), 18);
...
export const makeBuyOrder = async(provider, exchange, tokens, order, dispatch) => {
    ...
    const amountGet = ethers.utils.parseUnits(order.amount, 18) //the amount you buy
...
export const makeSellOrder = async(provider, exchange, tokens, order, dispatch) => {
    ...
    const amountGet = ethers.utils.parseUnits((order.amount * order.price).toString(), 18) //the amount you buy
