...
contract Exchange {
    ...
    event Trade(uint256 id, address user, address tokenGet, uint256 amountGet, 
        address tokenGive, uint256 amountGive, address creator, uint256 timestamp);
    ...
    function _trade(
        ...
        uint256 _amountGive)
     internal {
        //Fee is paid by the user that fulfilled the order (msg.sender)
        ...
        emit Trade(_orderId, msg.sender, _tokenGet, _amountGet, 
            _tokenGive, _amountGive, _user, block.timestamp);
    }
